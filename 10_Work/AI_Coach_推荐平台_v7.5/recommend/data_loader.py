...
class DataLoader:
    ...
    2. Excel知识库（output/ 目录）：
       - 海克斯大乱斗符文知识库_v5.xlsx  → 符文基础信息+UGC+套装
    ...
    def _fallback_augment_stats_from_excel(self):
        ...
        excel_path = os.path.join(OUTPUT_DIR, "海克斯大乱斗符文知识库_v5.xlsx")
    ...
    def _calc_normalization_params(self):
        """
        基于真实数据计算归一化参数，给用户参考
        不自动覆盖（保持手动可调），但会记录推荐值供展示
        
        归一化参数说明:
        - WR_FLOOR / WR_CEILING: 胜率归一化下限/上限
        - PR_SATURATION: 选率饱和点
        - 当前参数基于Step1.1单符文全局数据计算
        """
        import numpy as np
        from recommend import scoring_engine as se_module

        if not self.augment_stats:
            return
        ...
        # 推荐的归一化参数（基于真实数据分位数）
        # 胜率：下限=P5, 上限=P95（覆盖90%的数据范围）
        wr_floor_rec = round(float(np.percentile(wrs, 5)), 1)
        wr_ceiling_rec = round(float(np.percentile(wrs, 95)), 1)
        # 选率：饱和点=P90（90%的符文选率都在此以下）
        pr_sat_rec = round(float(np.percentile(prs, 90)), 2)
        ...
        self.normalization_stats = {
            ...
            "recommended": {
                "WR_FLOOR": wr_floor_rec,
                "WR_CEILING": wr_ceiling_rec,
                "PR_SATURATION": pr_sat_rec,
            },
            "current": {
        ...
        logger.info(f"  归一化参数参考(基于Step1.1): "
                    f"胜率[{wrs.min():.1f}%~{wrs.max():.1f}%] → "
                    f"推荐下限P5={wr_floor_rec}%, 上限P95={wr_ceiling_rec}% | "
                    f"当前: {se_module.WR_FLOOR}%~{se_module.WR_CEILING}%")
        logger.info(f"  选率参考: P90={pr_sat_rec}% | 当前饱和点: {se_module.PR_SATURATION}%")
