...
"""
评分引擎模块 v3.3
实现：归一化 → 加权打分 → 英雄胜率纠偏 → 分英雄自适应阈值 → 新标签体系
     → 连胜/连败权重调整 → 娱乐符文特殊逻辑
...
# ==================== 权重配置 ====================
WEIGHT_PROFILES = {
    "standard": {"W_winrate": 0.60, "W_pickrate": 0.15, "W_ugc": 0.25},
    "winning":  {"W_winrate": 0.40, "W_pickrate": 0.15, "W_ugc": 0.45},   # ≥3连胜
    "losing":   {"W_winrate": 0.75, "W_pickrate": 0.15, "W_ugc": 0.10},   # ≥3连败
}
...
class ScoringEngine:
    ...
    公式: 最终得分 = (基础分 + 英雄纠偏分) + 黑科技加成
    基础分 = 符文胜率分×W_winrate + 符文选择率分×W_pickrate + UGC评分分×W_ugc
    英雄纠偏分 = (全英雄平均胜率 - 英雄胜率) × 纠偏强度系数 × 100 / (WR_CEILING - WR_FLOOR)
    ...
    def calc_final_score(self, augment_name, champion_id=None, streak=0,
        ...
        capped_bonus = min(blacktech_bonus, BLACKTECH_BONUS_CAP)

        final_score = corrected_score * multiplier + capped_bonus
        ...
        detail["blacktech_bonus"] = capped_bonus
