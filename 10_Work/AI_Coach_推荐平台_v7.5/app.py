...
def apply_params(params):
    ...
    se_module.PR_SATURATION = params["PR_SATURATION"]
...
def calc_hero_scores(hero_name, streak=0):
    ...
    for level, augs in AUGMENTS_BY_LEVEL.items():
        ...
        # 3. 强力单卡：如果推荐选取的卡中，最佳拍档+潜力组合不足3张，
        #    则从未打标签的TOP15%中补充第1-2名标上强力单卡
        rec_items = [it for it in items if it["logo"] == "推荐选取"]

        # 先统计推荐选取中已有标签的数量
        tagged_rec_count = sum(1 for it in rec_items
                               if it["tag"] in ("最佳拍档", "潜力组合"))

        # 如果推荐选取中有标签的不足3张，补充强力单卡
        if tagged_rec_count < 3:
            need_strong = min(2, 3 - tagged_rec_count)  # 最多补2张
            # 从推荐选取中找未打标签的、且在TOP15%内的
            strong_candidates = []
            for it in rec_items:
                if it["tag"] is None:
                    if engine._is_strong_card_for_hero(it["aug"], hero_id):
                        strong_candidates.append(it)
            # 取分数最高的need_strong个
            strong_candidates.sort(key=lambda x: -x["score"])
            for it in strong_candidates[:need_strong]:
                it["tag"] = "强力单卡"
...
@app.route("/")
def index():
    return send_from_directory("output", "recommend_platform_v7.5.html")
...
def api_reset_params():
    ...
    CURRENT_PARAMS.update({
        "W_winrate": 0.60, "W_pickrate": 0.15, "W_ugc": 0.25,
        "W_winrate_winning": 0.40, "W_pickrate_winning": 0.15, "W_ugc_winning": 0.45,
        "W_winrate_losing": 0.75, "W_pickrate_losing": 0.15, "W_ugc_losing": 0.10,
        "WR_FLOOR": 45.0, "WR_CEILING": 70.0, "PR_SATURATION": 3.0, "UGC_MAX": 10.0,
        "BLACKTECH_BONUS_CAP": 20, "HERO_CORRECTION_STRENGTH": 0.3,
        "HERO_CORRECTION_MAX": 8.0, "HERO_CORRECTION_MIN": -5.0,
        "TARGET_RECOMMEND_PER_LEVEL": 4, "MIN_RECOMMEND_PER_LEVEL": 2,
...
def api_simulate():
    ...
    # 标签可见性
    rec_items = [it for it in items if it["logo"] == "推荐选取"]
    tagged_rec_count = sum(1 for it in rec_items if it["tag"] in ("最佳拍档", "潜力组合"))
    if tagged_rec_count < 3:
        need_strong = min(2, 3 - tagged_rec_count)
        strong_candidates = []
        for it in rec_items:
            if it["tag"] is None:
                if engine._is_strong_card_for_hero(it["aug"], hero_id):
