...
class BlacktechMatcher:
    ...
    def match(self, augment_name, champion_name, stage, selected_augments):
        ...
        Returns:
            {
                "bonus": int,          # 黑科技加成分(纯黑科技,不含套装)
                "tag": str,            # 标签: "黑科技"/"强牌"/"趣味"
                "pitch": str,          # ≤10字推荐话术
                "details": list,       # 匹配详情
                "synergy_bonus": int,  # 套装羁绊加成(独立计算)
        ...
        for syn in self.dl.synergies:
            ...
            if len(overlap) >= 1:
                ...
                syn_bonus = 0
                # 逐件累加
                for piece_num in range(1, len(overlap) + 1):
                    tier_key = f"{piece_num}件"
                    if piece_num == 1:
                        # 第1件始终+5（还没触发层级效果）
                        syn_bonus += 5
                    elif tier_key in tier_effects and tier_effects[tier_key]:
                        # 该件数有层级效果 → +10
                        syn_bonus += 10
                    else:
                        # 该件数没有层级效果 → +5
        ...
        # 注意：bonus 不再加上 synergy_bonus，两者在 scoring_engine 中分别 cap
        ...
        # 黑科技bonus和套装synergy_bonus分开返回，各自在scoring_engine中独立cap
        # bonus: 纯黑科技加成（专属+通用组合），不含套装
        # synergy_bonus: 纯套装加成
        return {
            "bonus": bonus,
            "tag": tag,
